...
const Dashboard = () => {
  ...
  const { projects, error, user } = state;
  ...
  // Fetch projects when user changes (including impersonation)
  ...
  }, [user, lastUserId, actions]);

  // Also refresh projects when impersonation state changes
  const isImpersonating = actions.isImpersonating();
  const [lastImpersonationState, setLastImpersonationState] = useState(false);
  useEffect(() => {
    if (isImpersonating !== lastImpersonationState) {
      setLastImpersonationState(isImpersonating);
  ...
  }, [isImpersonating, lastImpersonationState, user, actions]);
